...
local function run(msg, matches)
  ...
  if matches[1] == '!plugins' and is_momod(msg) then --after changed to moderator mode, set only sudo
    return list_all_plugins()
  end

  -- Re-enable a plugin for this chat
  if matches[1] == '+' and matches[3] == 'datak' then
  ...
  -- Enable a plugin
  if matches[1] == '+' and is_datak(msg) then --after changed to moderator mode, set only sudo
      if is_momod(msg) then
  ...
  if matches[1] == '-' and is_datak(msg) then --after changed to moderator mode, set only sudo
    if matches[2] == 'p' then
    	return 'This plugin cant be disabled'
    end
  ...
  if matches[1] == '*' and is_momod(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true)
  end
  if matches[1] == '*' and is_momod(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true)
  end
 if matches[1] == "=" and is_datak(msg) then
text = io.popen("cd plugins && rm "..matches[2]..".lua")
return "Plugins ["..matches[2].."] is DeleTed"
end 
end
return {
